...
@mock_s3
def test_cli_train_without_model_key_with_local_output():
    # Should default to local model path
    # Arrange
    from nba.src.cli import parse_args, run
    from nba.src.models.nba_lstm_branched import PlayerEmbeddingLSTM
    ...
    test_args = ["train", "--input-file", sample_input_file, "--local-dir", DEFAULT_TEST_DIR]

    # Act
    args = parse_args(test_args)
    results = run(args)
    trained_model_state_dict = torch.load(results["output"])
